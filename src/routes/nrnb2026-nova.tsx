...
export function NewNrnbLanding({ page = "/nrnb2026-nova" }: { page?: string }) {
  ...
  return <main className="nrnb2026-nova-page min-h-screen overflow-x-clip bg-background pb-16 lg:pb-0"><Ticker/>{campaign&&<Navigation/>}<Hero campaign={campaign}/>{!campaign&&<Navigation/>}<Identification campaign={campaign}/><Presentation campaign={campaign}/><Benefits/><Topics campaign={campaign}/><EventExperience/><Speakers/><Schedule/><SocialProof/><Fair/><Tickets campaign={campaign}/><AboutSuelen/><Closing/><Faq/><FinalCTA/><Footer/><StickyCTA/><WhatsApp/></main>;
}
