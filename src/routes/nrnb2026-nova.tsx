...
function CTA({ children, event = "benefits_cta_click", className = "" }: { children: React.ReactNode; event?: "hero_cta_click" | "benefits_cta_click" | "final_cta_click" | "sticky_cta_click"; className?: string }) {
  return <Button type="button" size="lg" onClick={() => { track(event, { variant: VARIANT }); goTo("ingressos"); }} className={`h-auto min-h-12 rounded-md bg-foreground px-7 py-3.5 text-center text-sm font-bold uppercase text-background shadow-card hover:bg-foreground/85 ${className}`}>{children}<ArrowRight /></Button>;
}
...
function Hero() {
  return <section className="hero-scene relative min-h-[calc(100svh-38px)] overflow-hidden"><div className="absolute inset-0"><img src={heroImage.url} alt="Suelen Gubeisse e especialistas do Não Repara na Bagunça 2026" fetchPriority="high" className="h-full w-full object-cover object-[68%_center] opacity-75" /><div className="absolute inset-0 bg-gradient-to-r from-background via-background/90 to-background/20" /><div className="absolute inset-0 bg-gradient-to-t from-background via-transparent to-transparent lg:hidden" /></div><div className="relative mx-auto flex min-h-[calc(100svh-38px)] max-w-7xl items-end px-5 pb-8 pt-20 sm:px-6 lg:items-center lg:py-16"><div className="max-w-2xl"><img src={logoNrnb.url} alt="Não Repara na Bagunça" width={546} height={187} className="w-[190px] sm:w-[260px]" /><p className="mt-5 text-sm font-bold uppercase tracking-[0.14em] text-sky-highlight">24 e 25 de outubro • São José dos Campos</p><h1 className="mt-4 text-balance text-4xl italic leading-[1.05] text-foreground sm:text-6xl">Dois dias para aprender a organizar melhor sua casa, sua rotina e sua vida.</h1><p className="mt-5 max-w-xl text-base leading-relaxed text-muted-foreground sm:text-lg">Leve para a sua vida práticas de organização que ajudam a tornar a rotina mais simples, a casa mais funcional e a liberar tempo para o que também importa: você.</p><div className="mt-5 grid gap-2 text-sm font-semibold sm:grid-cols-3"><span className="border-l-2 border-sky pl-3">2 dias de experiência</span><span className="border-l-2 border-sky pl-3">Especialistas e palestrantes</span><span className="border-l-2 border-sky pl-3">Casa • Rotina • Bem-estar</span></div><div className="mt-7"><CTA event="hero_cta_click">QUERO VIVER ESSA EXPERIÊNCIA</CTA><p className="mt-3 text-sm text-muted-foreground">Escolha seu ingresso para os dois dias do evento.</p></div><button type="button" onClick={() => goTo("experiencia")} aria-label="Continuar conhecendo o evento" className="mt-6 inline-flex items-center gap-2 text-sm text-muted-foreground"><ArrowDown className="h-4 w-4" /> Continue</button></div></div></section>;
}

function Navigation() {
  const [open, setOpen] = useState(false);
  const links = [["Experiência","experiencia"],["Palestrantes","palestrantes"],["Programação","programacao"],["Ingressos","ingressos"],["FAQ","faq"]];
  return <nav aria-label="Navegação da página" className="sticky top-0 z-40 border-b border-border bg-surface/95 px-4 py-3 shadow-card backdrop-blur"><div className="mx-auto grid max-w-6xl grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3"><img src={logoNrnb.url} alt="NRNB" className="w-24" /><div className="hidden justify-center gap-6 lg:flex">{links.map(([label,id]) => <button key={id} type="button" onClick={() => goTo(id)} className="text-sm font-semibold text-muted-foreground hover:text-primary">{label}</button>)}</div><div className="flex justify-end gap-2"><Button type="button" size="sm" onClick={() => goTo("ingressos")} className="bg-primary text-primary-foreground">COMPRAR</Button><Button type="button" size="icon" variant="outline" className="lg:hidden" aria-label={open ? "Fechar menu" : "Abrir menu"} onClick={() => setOpen(v => !v)}>{open ? <X /> : <Menu />}</Button></div></div>{open && <div className="mx-auto mt-3 grid max-w-6xl border-t border-border pt-3 lg:hidden">{links.map(([label,id]) => <button key={id} type="button" onClick={() => { setOpen(false); goTo(id); }} className="border-b border-border py-3 text-left text-sm font-semibold">{label}</button>)}</div>}</nav>;
}
...
function Footer() { return <footer className="bg-foreground px-5 py-10 text-background"><div className="mx-auto grid max-w-6xl gap-7 text-center sm:grid-cols-[1fr_auto] sm:text-left"><div><img src={logoNrnb.url} alt="Não Repara na Bagunça" className="mx-auto w-40 sm:mx-0"/><p className="mt-3 text-sm text-background/70">Não Repara na Bagunça 2026</p></div><div className="flex flex-wrap items-center justify-center gap-5 text-sm"><a suppressHydrationWarning href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer">Contato</a></div></div><p className="mx-auto mt-7 max-w-6xl border-t border-background/20 pt-5 text-center text-sm text-background/60">© 2026 Não Repara na Bagunça. Todos os direitos reservados.</p></footer>; }
